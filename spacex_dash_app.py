...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df,
                     names='class',
                     title='Success Launch on '+entered_site)
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, payload_slider):
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df.loc[spacex_df['Launch Site'] == entered_site]
        new_filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= payload_slider[0])
                                      & (filtered_df['Payload Mass (kg)'] <= payload_slider[1])]
        fig = px.scatter(new_filtered_df, x="Payload Mass (kg)", y="class",
                         color="Booster Version Category")
        return fig
